 ...
 * v3.2 — Controles táctiles, rotación corregida y UI responsiva
 * ============================================================
 */
...
(function () {
  ...
  const cfg = Object.assign(
    {
      ...
      autoRotate:      false,
      autoRotateSpeed: 0.8, // Velocidad aumentada para que sea notable
      debug:           true,
      upVector:        [0, 1, 0],
      subtitle:        "EXTERIOR",
  ...
  const ICONS = {
    ...
    exterior: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>`,
    interior: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"></path><path d="M2 12h20"></path></svg>`
  };
  ...
  const CSS = `
    ...
    .omh-bottom-left { display: flex; gap: 8px; align-items: center; position: absolute; bottom: 30px; left: 40px; }
    .omh-divider { width: 1px; height: 16px; background: rgba(255,255,255,0.15); margin: 0 5px; }
    .omh-bottom-right { position: absolute; bottom: 30px; right: 40px; }

    .omh-btn-rect { pointer-events: auto; background: rgba(0,0,0,0.4); backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.15); border-radius: 4px; color: rgba(255,255,255,0.6); font-family: 'Lexend Tera', sans-serif; font-size: 0.55rem; letter-spacing: 0.15em; font-weight: bold; padding: 10px 16px; display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer; transition: all 0.2s ease; text-transform: uppercase; }
    ...
    .omh-btn-sq { pointer-events: auto; width: 38px; height: 38px; background: rgba(0,0,0,0.4); backdrop-filter: blur(4px); border: 1px solid rgba(255,255,255,0.15); border-radius: 4px; color: rgba(255,255,255,0.6); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s ease; }
    ...
    /* LÓGICA RESPONSIVA (MÓVILES) */
    @media (max-width: 768px) {
      .omh-ui-layer { padding: 15px; }
      .omh-top-left { left: 15px; top: 15px; }
      .omh-top-right { top: 15px; right: 15px; gap: 6px; }
      .omh-bottom-left { bottom: 15px; left: 15px; flex-wrap: wrap; gap: 6px; }
      .omh-bottom-right { bottom: 15px; right: 15px; }
      .omh-divider { display: none; }
      .omh-top-left .omh-tit { font-size: 1.1rem; }
      
      /* Ocultar texto y hacer cuadrados los botones en móvil */
      .omh-btn-text { display: none; }
      .omh-btn-rect { padding: 0; width: 38px; height: 38px; }
      .omh-btn-rect svg { width: 16px; height: 16px; }
      /* Mantener el botón de FOV un poco más ancho si necesitamos ver el número */
      #btn-fov { width: auto; padding: 0 10px; }
      #btn-fov .omh-btn-text { display: inline-block; } 
    }
  `;
  ...
  const VS = `
    ...
    attribute vec2  a_uv; attribute vec3  a_center; attribute vec2  a_axes0; attribute vec2  a_axes1; attribute vec4  a_color;     
    ...
    void main() {
      ...
      offset = offset / u_vp * 2.0; posClip.xy += offset * posClip.w;
      gl_Position = posClip;
    }
  `;
  ...
  const FS = `
    ...
    void main() {
      ...
      float alpha = exp(-3.0 * r2) * v_color.a; if (alpha < 0.004) discard;
  ...
  class SplatRenderer {
    constructor(canvas) {
      ...
      this._buildProgram();
      this.splatCount = 0; this.vbo = gl.createBuffer();
      this.camera = { theta: cfg.defaultTheta, phi: cfg.defaultPhi, radius: 4, target: [0, 0, 0], fov: cfg.defaultFov };
      this._defaultCam = null; this.dirty = true;
    }
    ...
    async load(url, onProgress) {
      ...
      let resp; try { resp = await fetch(url); } catch (e) { throw new Error("Error de conexión. " + e.message); }
      if (!resp.ok) throw new Error(`Error HTTP ${resp.status}`);

      const contentLength = parseInt(resp.headers.get("content-length") || "0"), reader = resp.body.getReader(), chunks = [];
      let loaded = 0;
      while (true) {
      ...
      const buffer = new ArrayBuffer(loaded), view = new Uint8Array(buffer); let offset = 0;
      for (const c of chunks) { view.set(c, offset); offset += c.byteLength; }
      if (onProgress) onProgress(0.85);

      const ext = url.split(".").pop().toLowerCase(); let splats;
      if (ext === "splat") splats = this._parseSplat(buffer); else if (ext === "ply") splats = this._parsePly(buffer); else throw new Error(`Formato no soportado.`);

      if (onProgress) onProgress(0.9);
      this._fitCamera(splats); this._buildGeometry(splats);
    ...
    _parsePly(buffer) {
      ...
      const propIdx = name => props.findIndex(p => p.name === name), SIZES = { float: 4, double: 8, int: 4, uint: 4, short: 2, ushort: 2, char: 1, uchar: 1 }, stride = props.reduce((s, p) => s + (SIZES[p.type] || 4), 0), offsets = []; let cur = 0;
      for (const p of props) { offsets.push(cur); cur += (SIZES[p.type] || 4); }
      const count = vertCount, pos = new Float32Array(count * 3), scale = new Float32Array(count * 3), color = new Float32Array(count * 4), rot = new Float32Array(count * 4), xi = propIdx("x"), yi = propIdx("y"), zi = propIdx("z"), s0i = propIdx("scale_0"), s1i = propIdx("scale_1"), s2i = propIdx("scale_2"), r0i = propIdx("rot_0"), r1i = propIdx("rot_1"), r2i = propIdx("rot_2"), r3i = propIdx("rot_3"), dc0 = propIdx("f_dc_0"), dc1 = propIdx("f_dc_1"), dc2 = propIdx("f_dc_2"), oi = propIdx("opacity"), SH_C0 = 0.28209479177387814;
      if (isBinary) {
        const dv = new DataView(buffer, dataStart), getF = (base, pi) => { if (pi < 0) return 0; const t = props[pi].type, o = base + offsets[pi]; if (t === "float") return dv.getFloat32(o, true); if (t === "double") return dv.getFloat64(o, true); if (t === "uchar") return dv.getUint8(o); return dv.getFloat32(o, true); };
        for (let i = 0; i < count; i++) {
          const base = i * stride; pos[i*3] = getF(base, xi); pos[i*3+1] = getF(base, yi); pos[i*3+2] = getF(base, zi);
          ...
          if (r0i >= 0) { const qw = getF(base, r0i), qx = getF(base, r1i), qy = getF(base, r2i), qz = getF(base, r3i), len = Math.sqrt(qw*qw+qx*qx+qy*qy+qz*qz) || 1; rot[i*4] = qx/len; rot[i*4+1] = qy/len; rot[i*4+2] = qz/len; rot[i*4+3] = qw/len; } else { rot[i*4+3] = 1; }
        }
      } else {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (r0i >= 0) { const qw=vals[r0i], qx=vals[r1i], qy=vals[r2i], qz=vals[r3i], len = Math.sqrt(qw*qw+qx*qx+qy*qy+qz*qz) || 1; rot[i*4]=qx/len; rot[i*4+1]=qy/len; rot[i*4+2]=qz/len; rot[i*4+3]=qw/len; } else { rot[i*4+3] = 1; }
        }
      }
      return { count, pos, scale, color, rot };
    }
    ...
    _fitCamera({ count, pos }) {
      ...
      this.camera.target = [cx, cy, cz]; this.camera.radius = size * 2.0;
      this._defaultCam = { target: [cx, cy, cz], radius: size * 2.0, theta: cfg.defaultTheta, phi: cfg.defaultPhi };
    }

    resetCamera() {
      if (this._defaultCam) { this.camera.target = [...this._defaultCam.target]; this.camera.radius = this._defaultCam.radius; this.camera.theta = this._defaultCam.theta; this.camera.phi = this._defaultCam.phi; this.dirty = true; }
    }
    ...
    _buildGeometry({ count, pos, scale, color, rot }) {
      ...
      this._pendingSplats = { count, pos, scale, color, rot }; this.splatCount = count; this._geoNeedsRebuild = true; this._data = data;
    }
    ...
    _buildGeoWithView(view, vpW, vpH) {
      ...
      for (let ii = 0; ii < count; ii++) {
        const i = order[ii], px = pos[i*3], py = pos[i*3+1], pz = pos[i*3+2], vx = view[0]*px + view[4]*py + view[8]*pz + view[12], vy = view[1]*px + view[5]*py + view[9]*pz + view[13], vz = view[2]*px + view[6]*py + view[10]*pz + view[14];
        if (vz >= 0) { for (let v = 0; v < VERTS; v++) data[(ii*VERTS+v)*FPV + 4] = 0; continue; }

        const qx=rot[i*4], qy=rot[i*4+1], qz=rot[i*4+2], qw=rot[i*4+3], r00=1-2*(qy*qy+qz*qz), r01=2*(qx*qy-qw*qz), r02=2*(qx*qz+qw*qy), r10=2*(qx*qy+qw*qz), r11=1-2*(qx*qx+qz*qz), r12=2*(qy*qz-qw*qx), r20=2*(qx*qz-qw*qy), r21=2*(qy*qz+qw*qx), r22=1-2*(qx*qx+qy*qy), sx=scale[i*3], sy=scale[i*3+1], sz=scale[i*3+2], m00=r00*sx, m01=r01*sy, m02=r02*sz, m10=r10*sx, m11=r11*sy, m12=r12*sz, m20=r20*sx, m21=r21*sy, m22=r22*sz, w00=view[0]*m00+view[4]*m10+view[8]*m20, w01=view[0]*m01+view[4]*m11+view[8]*m21, w02=view[0]*m02+view[4]*m12+view[8]*m22, w10=view[1]*m00+view[5]*m10+view[9]*m20, w11=view[1]*m01+view[5]*m11+view[9]*m21, w12=view[1]*m02+view[5]*m12+view[9]*m22, w20=view[2]*m00+view[6]*m10+view[10]*m20, w21=view[2]*m01+view[6]*m11+view[10]*m21, w22=view[2]*m02+view[6]*m12+view[10]*m22, s00=w00*w00+w01*w01+w02*w02, s01=w00*w10+w01*w11+w02*w12, s11=w10*w10+w11*w11+w12*w12, tz = -vz, J00 = fx * vpW / 2 / tz, J11 = fy * vpH / 2 / tz, c00 = J00 * J00 * s00, c01 = J00 * J11 * s01, c11 = J11 * J11 * s11, det = c00 * c11 - c01 * c01, tr = c00 + c11, disc = Math.max(0, tr * tr / 4 - det), l1 = tr / 2 + Math.sqrt(disc), l2 = Math.max(0, tr / 2 - Math.sqrt(disc));

        let ax0, ay0, ax1, ay1;
        if (Math.abs(c01) > 0.0001) { const len0 = Math.sqrt((l1 - c11) ** 2 + c01 * c01) || 1; ax0 = (l1 - c11) / len0; ay0 = c01 / len0; } else { ax0 = 1; ay0 = 0; }
        ax1 = -ay0; ay1 = ax0; const r1 = Math.min(3 * Math.sqrt(l1), Math.max(vpW, vpH) * 0.5), r2 = Math.min(3 * Math.sqrt(l2), Math.max(vpW, vpH) * 0.5), A0x = ax0 * r1, A0y = ay0 * r1, A1x = ax1 * r2, A1y = ay1 * r2, cr = color[i*4], cg = color[i*4+1], cb = color[i*4+2], ca = color[i*4+3];
      ...
      const gl = this.gl; gl.bindBuffer(gl.ARRAY_BUFFER, this.vbo); gl.bufferData(gl.ARRAY_BUFFER, data, gl.DYNAMIC_DRAW); this._geoNeedsRebuild = false; this._lastView = view.slice();
    }

    _perspective(fov, aspect, near, far) { const f = 1 / Math.tan(fov * Math.PI / 360), nf = 1 / (near - far); return new Float32Array([ f/aspect, 0, 0, 0, 0, f, 0, 0, 0, 0, (far+near)*nf, -1, 0, 0, 2*far*near*nf, 0 ]); }
    _lookAt(eye, center, up) { let [fx,fy,fz] = [center[0]-eye[0], center[1]-eye[1], center[2]-eye[2]]; let fl = Math.sqrt(fx*fx+fy*fy+fz*fz) || 1; fx/=fl; fy/=fl; fz/=fl; let [sx,sy,sz] = [fy*up[2]-fz*up[1], fz*up[0]-fx*up[2], fx*up[1]-fy*up[0]]; let sl = Math.sqrt(sx*sx+sy*sy+sz*sz) || 1; sx/=sl; sy/=sl; sz/=sl; const ux=sy*fz-sz*fy, uy=sz*fx-sx*fz, uz=sx*fy-sy*fx; return new Float32Array([ sx, ux, -fx, 0, sy, uy, -fy, 0, sz, uz, -fz, 0, -(sx*eye[0]+sy*eye[1]+sz*eye[2]), -(ux*eye[0]+uy*eye[1]+uz*eye[2]), fx*eye[0]+fy*eye[1]+fz*eye[2], 1 ]); }
    _eye() { const { theta, phi, radius, target } = this.camera; return [ target[0] + radius * Math.sin(phi) * Math.sin(theta), target[1] + radius * Math.cos(phi), target[2] + radius * Math.sin(phi) * Math.cos(theta) ]; }

    render() {
      if (!this.splatCount) return; const gl = this.gl, { width: W, height: H } = this.canvas;
  ...
  function init() {
      ...
      <div class="omh-wrap" id="omh-wrap" style="height:${cfg.height}">
        ...
        <div class="omh-ui-layer" id="omh-ui-layer">
          ...
          <div class="omh-bottom-left">
            <button class="omh-btn-rect" id="btn-rst">${ICONS.reset} <span class="omh-btn-text">Centrar</span></button>
            <button class="omh-btn-rect" id="btn-ar">${ICONS.autorot} <span class="omh-btn-text">Auto-rotar</span></button>
            <button class="omh-btn-rect" id="btn-fov">${ICONS.fov} <span class="omh-btn-text">FOV: ${cfg.defaultFov}°</span></button>
            <div class="omh-divider"></div>
            <button class="omh-btn-rect active" id="btn-exterior">${ICONS.exterior} <span class="omh-btn-text">Exterior</span></button>
            <button class="omh-btn-rect" id="btn-interior">${ICONS.interior} <span class="omh-btn-text">Interior</span></button>
          </div>
          <div class="omh-bottom-right">
            <button class="omh-btn-rect orange" id="btn-tour">${ICONS.tour} <span class="omh-btn-text">Tour 360°</span></button>
          </div>
        </div>
    ...
    function showReady() { loader.classList.add("out"); uiLayer.classList.add("show"); hint.classList.add("show"); setTimeout(() => hint.classList.remove("show"), 3500); }
    if (!cfg.modelPath) { loader.classList.add("out"); errBox.classList.add("show"); errMsg.textContent = "Configura modelPath"; return; }

    renderer.load(cfg.modelPath, (p) => { const v = Math.round(p * 100); bar.style.width = v + "%"; pctEl.textContent = v + "%"; }).then(showReady).catch(e => { loader.classList.add("out"); errBox.classList.add("show"); errMsg.textContent = "Verifica la ruta del archivo."; });

    document.getElementById("btn-rst").addEventListener("click", () => { renderer.resetCamera(); renderer._geoNeedsRebuild = true; });
    
    const btnFov = document.getElementById("btn-fov"), fovValues = [35, 55, 75, 90];
    btnFov.addEventListener("click", () => {
      let currIdx = fovValues.indexOf(renderer.camera.fov); if (currIdx === -1) currIdx = 1;
      let nextIdx = (currIdx + 1) % fovValues.length; renderer.camera.fov = fovValues[nextIdx];
      btnFov.innerHTML = `${ICONS.fov} <span class="omh-btn-text">FOV: ${renderer.camera.fov}°</span>`;
      renderer.dirty = true; renderer._geoNeedsRebuild = true;
    });

    let autoRot = cfg.autoRotate; const btnAr = document.getElementById("btn-ar");
    ...
    document.addEventListener("fullscreenchange", () => {
      const full = !!document.fullscreenElement; wrap.classList.toggle("fullscreen", full);
      btnFs.innerHTML = full ? ICONS.exitfull : ICONS.fullscr; wrap.style.height = full ? "100vh" : cfg.height; setTimeout(resize, 80);
    });

    document.getElementById("btn-share").addEventListener("click", () => { navigator.clipboard.writeText(window.location.href); alert("Enlace copiado al portapapeles"); });
    ...
    // ── Controles Táctiles (Móviles) restaurados ──────────────
    let t0 = null, t1 = null, lastDist = 0;
    canvas.addEventListener("touchstart", e => {
      e.preventDefault(); const ts = e.touches; t0 = { x: ts[0].clientX, y: ts[0].clientY };
      if (ts.length >= 2) { t1 = { x: ts[1].clientX, y: ts[1].clientY }; lastDist = Math.hypot(t1.x-t0.x, t1.y-t0.y); }
    ...
    canvas.addEventListener("touchmove", e => {
      ...
      if (ts.length === 1 && t0) {
        const dx = ts[0].clientX - t0.x, dy = ts[0].clientY - t0.y; t0 = { x: ts[0].clientX, y: ts[0].clientY };
        renderer.camera.theta -= dx * 0.010; // Suavidad ajustada
        renderer.camera.phi = Math.max(0.05, Math.min(Math.PI - 0.05, renderer.camera.phi - dy * 0.010));
        renderer.dirty = true; renderer._geoNeedsRebuild = true;
      }
      if (ts.length >= 2) {
        const t0n = { x: ts[0].clientX, y: ts[0].clientY }, t1n = { x: ts[1].clientX, y: ts[1].clientY }, dist = Math.hypot(t1n.x-t0n.x, t1n.y-t0n.y);
        renderer.camera.radius = Math.max(0.05, renderer.camera.radius * (lastDist / dist)); lastDist = dist; t0 = t0n; t1 = t1n;
        renderer.dirty = true; renderer._geoNeedsRebuild = true;
      }
    }, { passive: false });

    // ── Loop Principal ────────────────────────────────────────
    let prevT = performance.now(); // Iniciar tiempo base correcto
    function loop(t) {
      requestAnimationFrame(loop);
      const dt = t - prevT; prevT = t;
      if (!renderer.splatCount) return;
      if (autoRot) { 
        // Lógica de rotación aumentada para que sea perceptible (0.8 * dt * 0.002)
        renderer.camera.theta += cfg.autoRotateSpeed * dt * 0.002 * (Math.PI / 180); 
        renderer.dirty = true; renderer._geoNeedsRebuild = true; 
      }
