...
(function () {
  ...
  function aplicarModoStandalone() {
    ...
    css.textContent = `
      body.v360-standalone > *:not(.omh-360-widget-standalone-wrap) { display: none !important; }
      .omh-360-widget-standalone-wrap { position: fixed !important; inset: 0 !important; width: 100vw !important; height: 100dvh !important; z-index: 99999 !important; background: #000; }
      .omh-360-widget-standalone-wrap .omh-360-widget { width: 100% !important; height: 100% !important; border-radius: 0 !important; border: none !important; margin: 0 !important; aspect-ratio: unset !important; }
    `;
  ...
  function inyectarDependencias(callback) {
    ...
    s.textContent = `
      ...
      @media (pointer: coarse) { .v360-btn-fs { display: none !important; } .v360-btn-giro { display: flex; } .v360-hud-logo { display: none !important; } .v360-hud-titulo { max-width: 110px; } }
  ...
  class OMHWidget360 {
    constructor(contenedor, opcionesExtra) {
      ...
      this.autoIniciar   = (opcionesExtra && opcionesExtra.autoIniciar) || (contenedor.getAttribute('data-autoiniciar') === 'true');
    ...
    construirDOM() {
      this.contenedor.innerHTML = `
        <div class="v360-container">
          ...
          <div class="v360-ficha-panel" id="ficha-${this.id}"><div class="v360-ficha-titulo">Ficha Comercial</div><div id="ficha-content-${this.id}"></div></div>
          <div class="v360-hud" id="hud-${this.id}">
            <div class="v360-hud-left-group">
              <button class="v360-btn" id="btn-salir-${this.id}" title="Salir del Recorrido" style="display: ${this.autoIniciar ? 'flex' : 'none'}; background: rgba(220,20,20,0.85); font-weight: bold; border-color: transparent;">✕</button>
              <img src="" class="v360-hud-logo" id="logo-${this.id}">
              <div class="v360-hud-info"><span class="v360-hud-cliente" id="cliente-${this.id}"></span><span class="v360-hud-titulo" id="titulo-${this.id}"></span></div>
            </div>
    ...
    _registrarInteraccionDrag(wrap) {
      ...
      wrap.addEventListener('mousedown', e => {
        ...
        this.autoRotar = false; document.getElementById('btn-rot-' + this.id).classList.remove('activo');
      });
      ...
      wrap.addEventListener('touchmove', e => {
        ...
        if (e.touches.length === 1) {
          ...
          if (this.giroscopioActivo && this._gyroActivo) {
            ...
            const rotX = new THREE.Quaternion().setFromAxisAngle(ejeXLocal, -dy * 0.003);
            this._gyroOffset.premultiply(rotY); this._gyroOffset.premultiply(rotX);
          } else { this.lon -= dx * 0.22; this.lat += dy * 0.22; }
    ...
    _registrarBotones(wrap) {
      document.getElementById('btn-rot-' + this.id).addEventListener('click', e => {
        this.autoRotar = !this.autoRotar; e.currentTarget.classList.toggle('activo', this.autoRotar);
        if (this.autoRotar && this.giroscopioActivo) { this.giroscopioActivo = false; document.getElementById('btn-giro-' + this.id).classList.remove('activo'); }
      });
      document.getElementById('btn-ficha-' + this.id).addEventListener('click', e => {
        this.fichaActiva = !this.fichaActiva; e.currentTarget.classList.toggle('activo', this.fichaActiva);
        document.getElementById('ficha-' + this.id).style.display = this.fichaActiva ? 'block' : 'none';
      });
      document.getElementById('btn-giro-' + this.id).addEventListener('click', () => this._solicitarPermisoGiroscopio(true));
      ...
      if (btnFS) {
        ...
        const syncIcon = () => {
          const enFS = !!(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement);
          btnFS.textContent = enFS ? '⮌' : '⛶'; btnFS.classList.toggle('activo', enFS);
        };
        ['fullscreenchange','webkitfullscreenchange','mozfullscreenchange','MSFullscreenChange'].forEach(ev => document.addEventListener(ev, syncIcon));
      }

      document.getElementById('sp-btn-' + this.id).addEventListener('click', () => this._activarVisor(wrap));
      document.getElementById('mm-toggle-' + this.id).addEventListener('click', () => document.getElementById('mm-wrap-' + this.id).classList.toggle('colapsado'));
    }
    ...
    _solicitarPermisoGiroscopio(mostrarAlertaError) {
      ...
      if (this.giroscopioActivo) {
        this.giroscopioActivo = false; this._gyroActivo = false; btnGiro.classList.remove('activo');
      ...
      if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        DeviceOrientationEvent.requestPermission().then(r => { if (r === 'granted') this._encenderGiroscopio(); else if (mostrarAlertaError) alert('Permiso denegado.'); }).catch(err => { if (mostrarAlertaError) alert('Interactúa primero con la pantalla.'); });
      } else { this._encenderGiroscopio(); }
    }

    _encenderGiroscopio() {
      this.giroscopioActivo = true; this.autoRotar = false; this._gyroActivo = false;
      ...
      this._eventoOrientacion = ('ondeviceorientationabsolute' in window) ? 'deviceorientationabsolute' : 'deviceorientation';
      window.removeEventListener('deviceorientationabsolute', this._onDeviceOrientationBind, true);
      window.removeEventListener('deviceorientation', this._onDeviceOrientationBind, true);
    ...
    _onDeviceOrientation(event) {
      if (!this.giroscopioActivo || this.faseTransicion !== 'quieto' || !this._gyroQ || event.alpha === null) return;
      const alpha  = THREE.MathUtils.degToRad(event.alpha), beta = THREE.MathUtils.degToRad(event.beta), gamma = THREE.MathUtils.degToRad(event.gamma), orient = THREE.MathUtils.degToRad(window.screen?.orientation?.angle ?? window.orientation ?? 0);
      this._gyroEuler.set(beta, alpha, -gamma, 'YXZ');
      this._gyroQ.setFromEuler(this._gyroEuler).multiply(this._gyroQ1).multiply(this._gyroQ0.setFromAxisAngle(this._gyroZee, -orient));
      if (!this._gyroActivo) { this._gyroOffset = this.camera.quaternion.clone().multiply(this._gyroQ.clone().invert()); this._gyroActivo = true; }
    }

    _toggleFullscreen() {
      const el  = this.contenedor;
      const enFS = !!(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement);
      if (!enFS) { const req = el.requestFullscreen || el.webkitRequestFullscreen || el.mozRequestFullScreen || el.msRequestFullscreen; if (req) req.call(el); } 
      else { const exit = document.exitFullscreen || document.webkitExitFullscreen || document.mozCancelFullScreen || document.msExitFullscreen; if (exit) exit.call(document); }
    }

    procesarClickEnFoto(e) {
      const wrap = document.getElementById('canvas-' + this.id), rect = wrap.getBoundingClientRect();
      ...
      if (hits.length > 0) {
        ...
        let pitch = 90 - (Math.acos(p.y / r) * (180 / Math.PI)), yaw = ((Math.atan2(p.z, p.x) * (180 / Math.PI)) % 360 + 360) % 360;
        this.lastClickCoords = { pitch: Math.round(pitch * 10) / 10, yaw: Math.round(yaw * 10) / 10 };
    ...
    cargarEscenaInicial(idEscena) {
      ...
      this._cargarTextura(datos.imagen, tex => {
        const vieja = this.matPrincipal.map; if (vieja && vieja !== tex) this._disposeTex(vieja);
        this.matPrincipal.map = tex; this.matPrincipal.needsUpdate = true;
        this.spherePrincipal.rotation.z = datos.correccionHorizonte ? datos.correccionHorizonte * (Math.PI / 180) : 0;
        const spin = document.getElementById('spin-' + this.id); if (spin) { spin.style.opacity = '0'; setTimeout(() => spin.style.display = 'none', 300); }
    ...
    dispararTransicionHacia(idSiguiente) {
      ...
      const datosSig = this.config.escenas[idSiguiente]; if (!datosSig) return;
      document.getElementById('hs-' + this.id).style.opacity = '0';
      const spin = document.getElementById('spin-' + this.id); if (spin) { spin.style.display = 'flex'; spin.style.opacity = '1'; }
      this._cargarTextura(datosSig.imagen, tex => {
        const geoClon = new THREE.SphereGeometry(495, 60, 40); geoClon.scale(-1, 1, 1);
        ...
        this.sphereClon.rotation.z = datosSig.correccionHorizonte ? datosSig.correccionHorizonte * (Math.PI / 180) : 0;
        this.scene.add(this.sphereClon);
        if (spin) { spin.style.opacity = '0'; setTimeout(() => spin.style.display = 'none', 300); }
        this.siguienteEscenaID = idSiguiente; this.faseTransicion = 'zoom_in'; this.transitionProgress = 0;
      });
    }

    finalizarDisolvencia() {
      const texAntigua = this.matPrincipal.map;
      this.matPrincipal.map = this.matClon.map; this.matPrincipal.needsUpdate = true;
      this.spherePrincipal.rotation.z = this.sphereClon.rotation.z; this.matPrincipal.opacity = 1;
      this.scene.remove(this.sphereClon); this.sphereClon.geometry.dispose(); this.matClon.dispose(); 
      if (texAntigua && texAntigua !== this.matPrincipal.map) this._disposeTex(texAntigua);
      this.escenaActual = this.siguienteEscenaID;
      const datos = this.config.escenas[this.escenaActual];
      document.getElementById('titulo-' + this.id).textContent = datos.tituloEscena || '';
      if (this.giroscopioActivo) this.calibrandoGiroscopio = true; 
    ...
    renderizarHotspots(hotspots) {
      const layer = document.getElementById('hs-' + this.id); layer.innerHTML = ''; this.hotspotsData = [];
      hotspots.forEach(hs => {
        const el = document.createElement('div'); el.className = 'v360-hotspot'; const op = hs.opacidad !== undefined ? hs.opacidad : 1;
        let iconHTML = '';
        if (hs.iconoPng) { iconHTML = '<img src="' + hs.iconoPng + '" style="width:' + (hs.tamano||32) + 'px;height:auto;display:block;" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'block\';"><span style="display:none;font-size:' + (hs.tamano||28) + 'px;">' + (hs.tipo==='navegacion'?'➔':'📍') + '</span>'; } 
        else if (hs.icono) { iconHTML = '<span style="font-size:' + (hs.tamano||28) + 'px;filter:drop-shadow(0 2px 5px rgba(0,0,0,0.5));">' + hs.icono + '</span>'; } 
        else { const def = '../assets/tours/icon_circulo.png'; iconHTML = '<img src="' + def + '" style="width:' + (hs.tamano||32) + 'px;height:auto;display:block;" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'block\';"><span style="display:none;font-size:' + (hs.tamano||28) + 'px;filter:drop-shadow(0 2px 5px rgba(0,0,0,0.5));">' + (hs.tipo==='navegacion'?'➔':'📍') + '</span>'; }
        el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;opacity:' + op + ';transition:opacity 0.2s;">' + iconHTML + '</div>' + (hs.etiqueta && hs.tipo==='navegacion' ? '<div style="position:absolute;bottom:-22px;font-size:0.6rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;background:rgba(0,0,0,0.65);padding:2px 6px;border-radius:4px;white-space:nowrap;opacity:' + op + ';">' + hs.etiqueta + '</div>' : '');
        if (hs.tipo === 'navegacion') el.addEventListener('click', () => this.dispararTransicionHacia(hs.destino));
        else el.addEventListener('click', e => { this.mostrarTooltip(e, hs); e.stopPropagation(); });
        layer.appendChild(el);
        const phi = THREE.MathUtils.degToRad(90 - (hs.pitch || 0)), theta = THREE.MathUtils.degToRad(hs.yaw || 0);
        this.hotspotsData.push({ el, pos3D: new THREE.Vector3(500 * Math.sin(phi) * Math.cos(theta), 500 * Math.cos(phi), 500 * Math.sin(phi) * Math.sin(theta)) });
      });
    }

    mostrarTooltip(e, hs) {
      const tt = document.getElementById('tt-' + this.id), rect = e.currentTarget.getBoundingClientRect(), wrapRect = this.contenedor.getBoundingClientRect();
      ...
      tt.style.left = (rect.left - wrapRect.left + rect.width / 2) + 'px'; tt.style.top = (rect.top - wrapRect.top) + 'px'; tt.style.opacity = '1';
    }
    ocultarTooltip() { document.getElementById('tt-' + this.id).style.opacity = '0'; }
    toggleEditor(btn) { this.modoEditor = !this.modoEditor; btn.classList.toggle('activo', this.modoEditor); document.getElementById('panel-ed-' + this.id).style.display = this.modoEditor ? 'block' : 'none'; }
    copiarCoordenadas() { navigator.clipboard.writeText('pitch: ' + this.lastClickCoords.pitch + ',\nyaw: ' + this.lastClickCoords.yaw + ',').then(() => { const msg = document.getElementById('copy-msg-' + this.id); msg.textContent = '✓ Copiado'; setTimeout(() => msg.textContent = '', 2000); }); }

    construirMinimap() {
      const mm = document.getElementById('mm-' + this.id); if (!mm || !this.config.escenas) return; mm.innerHTML = '';
      Object.entries(this.config.escenas).forEach(([k, esc]) => {
        if (!esc.principalZona) return;
        const item = document.createElement('div'); item.className = 'v360-mm-escena'; item.dataset.zonaName = esc.zona || k;
        item.innerHTML = '<div class="v360-mm-dot"></div><span class="v360-mm-label">' + (esc.zona || k) + '</span>';
        item.addEventListener('click', () => this.dispararTransicionHacia(k)); mm.appendChild(item);
      });
      this.sincronizarMinimap();
    }
    sincronizarMinimap() {
      const esc = this.config.escenas[this.escenaActual]; if (!esc) return;
      this.contenedor.querySelectorAll('.v360-mm-escena').forEach(el => { el.classList.toggle('activa', el.dataset.zonaName === esc.zona); });
    }

    animar() {
      requestAnimationFrame(() => this.animar());
      if (this.faseTransicion === 'zoom_in') {
        this.transitionProgress += 0.045; const t = Math.min(1, this.transitionProgress), smooth = t * t * (3 - 2 * t);
        this.cameraFOV = THREE.MathUtils.lerp(this.cameraFOV, 73, 0.08);
        this.matPrincipal.opacity = 1 - smooth; this.matClon.opacity = smooth;
        ...
        if (Math.abs(this.cameraFOV - 75) < 0.1) { this.cameraFOV = 75; this.faseTransicion = 'quieto'; if (this.tourIniciado) document.getElementById('hs-' + this.id).style.opacity = '1'; }
      }
      this.camera.fov = this.cameraFOV; this.camera.updateProjectionMatrix();

      if (this.giroscopioActivo && this._gyroActivo && this.faseTransicion === 'quieto') {
        const qTarget = this._gyroOffset ? this._gyroOffset.clone().multiply(this._gyroQ) : this._gyroQ.clone();
      ...
      if (!this.giroscopioActivo || !this._gyroActivo) {
        const phi = THREE.MathUtils.degToRad(90 - this.lat), theta = THREE.MathUtils.degToRad(this.lon);
      ...
      this.hotspotsData.forEach(({ el, pos3D }) => {
        ...
        el.style.display = 'flex'; el.style.left = ((vec.x * 0.5 + 0.5) * W) + 'px'; el.style.top = ((vec.y * -0.5 + 0.5) * H) + 'px';
      });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (configPathStandalone) {
      ...
      const wrap = document.createElement('div'); wrap.className = 'omh-360-widget-standalone-wrap'; document.body.classList.add('v360-standalone'); document.body.appendChild(wrap);
      const widget = document.createElement('div'); widget.className = 'omh-360-widget'; widget.setAttribute('data-config', configPathStandalone); wrap.appendChild(widget);
      inyectarDependencias(() => new OMHWidget360(widget, { autoIniciar: true }));
    } else {
      const widgets = document.querySelectorAll('.omh-360-widget');
      if (widgets.length === 0) return;
      inyectarDependencias(() => {
        widgets.forEach(w => {
          // EXPORTAMOS GLOBALMENTE LA CREACIÓN SI ESTAMOS DENTRO DE SPLAT
          if (w.id === 'omh-360-interno') {
            w._omh360Diferido = true;
          } else {
            new OMHWidget360(w);
          }
        });
  ...
  // Hacemos accesibles estas variables al exterior para que ui.js pueda llamarlas a voluntad
  window.OMHWidget360 = OMHWidget360;
  window.inyectarDependenciasV360 = inyectarDependencias;
